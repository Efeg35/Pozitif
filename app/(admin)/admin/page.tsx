...
type CustomerRow = { full_name: string }
...
function resolveCustomerName(customers: CustomerRow | CustomerRow[] | null): string {
  if (!customers) return '—'
...
export default async function DashboardPage() {
  ...
  try {
    ...
    results.slice(0, 6).forEach(({ error }, i) => {
      if (error) console.error(`Dashboard query [${i}] failed:`, error.message)
    })
    ...
    if (!apptResult.error && apptResult.data) {
      upcomingAppointments = apptResult.data as unknown as UpcomingAppointment[]
    }
  } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatsCard title="Toplam İlan"    value={totalListings}         icon={Home}         description="Tüm durumlardaki ilanlar" />
        <StatsCard title="Aktif İlan"     value={activeListings}        icon={CheckCircle}  description="Sitede yayında" />
        <StatsCard title="Toplam Müşteri" value={totalCustomers}        icon={Users}        description="Kayıtlı müşteriler" />
        <StatsCard title="Bu Ay Randevu"  value={thisMonthAppointments} icon={CalendarDays} description="Bu aydaki randevular" />
      </div>
      ...
      {draftListings > 0 && (
        <div className="flex items-center justify-between gap-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <Link href="/admin/ilanlar?status=taslak" className="inline-flex items-center gap-1 text-xs font-medium text-amber-700 hover:text-amber-900 transition-colors whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <Link href="/admin/randevular" className="ml-auto inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 transition-colors">
                ...
                return (
                  <li key={appt.id} className="py-3 flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-zinc-900 truncate">{resolveCustomerName(appt.customers)}</p>
                      {appt.title && <p className="text-xs text-zinc-500 truncate">{appt.title}</p>}
                    </div>
        ...
        <div className="bg-white border rounded-lg p-6">
          ...
          {newInquiries > 0 ? (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <Link href="/admin/talepler" className="inline-flex items-center gap-1.5 text-sm font-medium text-blue-700 hover:text-blue-800 transition-colors">
                Talepleri görüntüle <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-10 text-zinc-400">
